//connecting to node
const alchemy =
  "https://eth-goerli.g.alchemy.com/v2/h_Yci2S6utW1IK8XRuJMMhq4Gs5QYLs6";
...
let ElonFunding = async () => {
  const addr = "0x859d3D6EB57AB05C2cd38a9c3c596e9dD46D60B4";
  n = 1;
  const contractSign = await same();

  //transferring ethers
  try {
    const eth = document.querySelectorAll("input")[0].value;
    const tx = await contractSign.SendEth(addr, {
...
let TotalFunds = async (addr, n) => {
  ...
  document.querySelectorAll("h5")[n].innerHTML =
    "Total Funds: " + ethers.utils.formatEther(total) + "ethers";
}
